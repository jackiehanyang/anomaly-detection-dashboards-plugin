...
import { analyzeLogsStatistically, LogAnalysisInsight, StatisticalAnalysisResult, LogEntry, generateQuestionBasedSummary } from './statisticalAnalysis';
import { LogPatternDetector, PatternDetectionResult, DetectedPattern } from './patternDetection';
...
/**
 * Main class for generating comprehensive insights from anomaly data
 */
export class AnomalyInsightGenerator {
  ...
  /**
   * Merge statistical insights and pattern detection results
   */
  private mergeInsights(
    statisticalInsights: LogAnalysisInsight[],
  ...
  private generateAnomalySpecificInsights(
    anomaly: AnomalyData,
    existingInsights: ComprehensiveInsight[]
  ): ComprehensiveInsight[] {
    const insights: ComprehensiveInsight[] = [];
   ...
   */
  private mapToCategory(type: string): ComprehensiveInsight['category'] {
    switch (type) {
      case 'pattern':
        return 'patterns';
      case 'statistical':
   ...
   */
  private mapPatternToCategory(patternType: string): ComprehensiveInsight['category'] {
    switch (patternType) {
      case 'error_sequence':
   ...
   */
  private generateSummary(insights: ComprehensiveInsight[]) {
...
/**
 * Utility function to format insights for display using question-based structure
 */
export function formatInsightsForDisplay(insights: ComprehensiveInsight[]): string {
  if (insights.length === 0) {
    return 'No significant patterns or issues detected in the logs.';
  }

  // Convert ComprehensiveInsight back to LogAnalysisInsight for the question-based summary
