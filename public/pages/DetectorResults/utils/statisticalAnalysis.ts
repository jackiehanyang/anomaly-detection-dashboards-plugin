...
import { AnomalyData, FeatureAggregationData } from '../../../models/interfaces';
...
export async function analyzeLogsStatistically(
  ...
  console.log('Starting advanced statistical analysis...');
  const advancedAnalysis = new AdvancedStatisticalAnalysis();
  
  try {
    // Extract numeric and categorical fields for analysis
    const numericFields = extractNumericFields(sortedLogs);
    const categoricalFields = extractCategoricalFields(sortedLogs);
    const allFields = [...numericFields, ...categoricalFields];
    
    console.log(`Found ${numericFields.length} numeric fields and ${categoricalFields.length} categorical fields`);
    
    // Always run advanced analysis, even with minimal fields
    console.log('Running advanced analysis algorithms...');
    ...
    console.log(`Enhanced fields: ${enhancedNumericFields.length} numeric, ${enhancedCategoricalFields.length} categorical`);
    ...
    console.log(`Found ${timeSeriesPatterns.length} time series patterns`);
    
    // Causal analysis
    const causalRelationships = advancedAnalysis.performCausalAnalysis(enhancedLogs, enhancedAllFields);
    console.log(`Found ${causalRelationships.length} causal relationships`);
    ...
    console.log(`Found ${clusters.length} anomaly clusters`);
    ...
    console.log(`Significance tests completed`);
    ...
    console.log(`Generated ${advancedInsights.length} advanced insights`);
  ...
  console.log('Insights by question:', {
    whatHappened: insights.filter(i => i.question === 'what_happened').length,
    rootCause: insights.filter(i => i.question === 'root_cause').length,
    patterns: insights.filter(i => i.question === 'patterns').length,
    recommendations: insights.filter(i => i.question === 'recommendations').length
  });
